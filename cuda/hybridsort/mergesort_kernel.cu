...
__global__ void
mergeSortFirst(float4 *result, int listsize, cudaTextureObject_t texObj) 
{
    // Block index
    int bx = blockIdx.x;
    // Thread index
...
__global__ void
mergeSortPass(float4 *result, int nrElems, int threadsPerDiv, cudaTextureObject_t texObj) 
{
