...
void hotspotOpt1(float *p, float* tIn, float *tOut, float sdc,
        ...
        float cn, float cs,
        float ct, float cb, 
        float cc, const sycl::nd_item<3> &item_ct1) 
{
    float amb_temp = 80.0;

    int i = item_ct1.get_local_range(2) * item_ct1.get_group(2) +
            item_ct1.get_local_id(2);
    int j = item_ct1.get_local_range(1) * item_ct1.get_group(1) +
        ...
        float dt, int numiter)
{
 ...
 sycl::queue &q_ct1 = dev_ct1.in_order_queue();
    ...
    q_ct1.memcpy(tIn_d, tIn, s);
    q_ct1.memcpy(p_d, p, s).wait();

    /*
    DPCT1026:0: The call to cudaFuncSetCacheConfig was removed because
 ...
 */
    ...
    for (int i = 0; i < numiter; ++i) {
        q_ct1.parallel_for(sycl::nd_range<3>(grid_dim * block_dim, block_dim),
    ...
    dpct::dpct_free(p_d, q_ct1);
    dpct::dpct_free(tIn_d, q_ct1);
    dpct::dpct_free(tOut_d, q_ct1);
    return;
}
