		...
		char** argv ) 
{
  ...
  printf("WG size of kernel 1 & 2  = %d \n", DEFAULT_ORDER);
	...
	int device = 0;
        /*
	DPCT1093:6: The "device" device may be not the one intended for
	...
	*/
	...
	// Print configuration
     ...
     if (commandFile==NULL) {printf ("Command File error",stderr); exit (1);}
     ...
     if (commandBuffer == NULL) {printf ("Command Buffer memory error",stderr); exit (2);}
     
     // copy the file into the buffer:
     result = fread (commandBuffer,1,lSize,commandFile);
     if (result != lSize) {printf ("Command file reading error",stderr); exit (3);}
