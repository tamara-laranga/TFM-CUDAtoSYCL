	...
	T* t;
        checkCudaErrors(
            DPCT_CHECK_ERROR(t = (T *)sycl::malloc_device(
                                 sizeof(T) * N, dpct::get_in_order_queue())));
        return t;
}
...
void dealloc(T* array)
{
        checkCudaErrors(DPCT_CHECK_ERROR(
            dpct::dpct_free((void *)array, dpct::get_in_order_queue())));
}
...
static dpct::constant_memory<sycl::double3, 1> ff_fc_momentum_x(1);
static dpct::constant_memory<sycl::double3, 1> ff_fc_momentum_y(1);
static dpct::constant_memory<sycl::double3, 1> ff_fc_momentum_z(1);
static dpct::constant_memory<sycl::double3, 1> ff_fc_density_energy(1);

SYCL_EXTERNAL void cuda_initialize_variables(int nelr, double *variables,
                                             const sycl::nd_item<3> &item_ct1,
                                             double const *ff_variable)
{
 ...
 * cuda_compute_flux_contributions exceeds 128 bytes and may cause high register
 * pressure. Consult with your hardware vendor to find the total register size
 * available and adjust the code, or use smaller sub-group size to avoid high
 * register pressure.
*/
void cuda_compute_flux_contributions(int nelr, double *variables,
                ...
                dpct::get_in_order_queue().parallel_for(
                    ...
                    [=](sycl::nd_item<3> item_ct1) {
                            cuda_compute_flux_contributions(
                                nelr, variables, fc_momentum_x, fc_momentum_y,
                                fc_momentum_z, fc_density_energy, item_ct1);
                    });
        }
                    /*
		    DPCT1010:43: SYCL uses exceptions to report
 ...
 * Consult with your hardware vendor to find the total register size available
 * and adjust the code, or use smaller sub-group size to avoid high register
 * pressure.
*/
void cuda_compute_flux(int nelr, int *elements_surrounding_elements,
                       ...
                       double *fluxes, const sycl::nd_item<3> &item_ct1,
                       double const *ff_variable,
                       sycl::double3 const *ff_fc_momentum_x,
                       sycl::double3 const *ff_fc_momentum_y,
                       sycl::double3 const *ff_fc_momentum_z,
                       sycl::double3 const *ff_fc_density_energy)
{
...
void compute_flux(int nelr, int* elements_surrounding_elements, double* normals, double* variables, double* fc_momentum_x, double* fc_momentum_y, double* fc_momentum_z, double* fc_density_energy, double* fluxes)
{
        ...
        dpct::err0 error;
        {
                ...
                dpct::get_in_order_queue().submit([&](sycl::handler &cgh) {
                        auto ff_variable_ptr_ct1 = ff_variable.get_ptr();
                        auto ff_fc_momentum_x_ptr_ct1 =
                            ff_fc_momentum_x.get_ptr();
                        auto ff_fc_momentum_y_ptr_ct1 =
                            ff_fc_momentum_y.get_ptr();
                        auto ff_fc_momentum_z_ptr_ct1 =
                            ff_fc_momentum_z.get_ptr();
                        ...
                        cgh.parallel_for(
                            ...
                            [=](sycl::nd_item<3> item_ct1) {
                                    cuda_compute_flux(
                                        nelr, elements_surrounding_elements,
                                        normals, variables, fc_momentum_x,
                                        fc_momentum_y, fc_momentum_z,
                                        fc_density_energy, fluxes, item_ct1,
                                        ff_variable_ptr_ct1,
...
int main(int argc, char** argv)
{
	...
	const char* data_file_name = argv[1];
        ...
        /*
	DPCT1093:49: The "0" device may be not the one intended for use.
	...
	*/
        ...
        checkCudaErrors(DPCT_CHECK_ERROR(dpct::get_device_info(
            prop, dpct::dev_mgr::instance().get_device(dev))));
        ...
        // set far field conditions and load them into constant memory on the gpu
	{
		...
		// copy far field conditions to the gpu
                ...
                checkCudaErrors(DPCT_CHECK_ERROR(
                    dpct::get_in_order_queue()
                        .memcpy(ff_fc_momentum_x.get_ptr(), &h_ff_fc_momentum_x,
                                sizeof(sycl::double3))
                        .wait()));
                checkCudaErrors(DPCT_CHECK_ERROR(
                    dpct::get_in_order_queue()
                        .memcpy(ff_fc_momentum_y.get_ptr(), &h_ff_fc_momentum_y,
                                sizeof(sycl::double3))
                        .wait()));
                checkCudaErrors(DPCT_CHECK_ERROR(
                    dpct::get_in_order_queue()
                        .memcpy(ff_fc_momentum_z.get_ptr(), &h_ff_fc_momentum_z,
                                sizeof(sycl::double3))
                        .wait()));
