 ...
 * kernel_ecc_2 exceeds 128 bytes and may cause high register pressure. Consult
...
SYCL_EXTERNAL void kernel_ecc_2(fp timeinst, fp *d_initvalu, fp *d_finavalu,
	...
	d_finavalu[offset_3] = aj*(1-d_initvalu_3)-bj*d_initvalu_3;
        I_Na_junc = Fjunc * GNa * dpct::pow(d_initvalu_1, 3) * d_initvalu_2 *
                    d_initvalu_3 * (d_initvalu_39 - ena_junc);
        I_Na_sl = Fsl * GNa * dpct::pow(d_initvalu_1, 3) * d_initvalu_2 *
	...
	// I_nak: Na/K Pump Current
        ...
        I_nak_junc = Fjunc * IbarNaK * fnak * Ko /
                     (1 + dpct::pow((KmNaip / d_initvalu_32), 4)) / (Ko + KmKo);
        I_nak_sl = Fsl * IbarNaK * fnak * Ko /
                   (1 + dpct::pow((KmNaip / d_initvalu_33), 4)) / (Ko + KmKo);
	...
	//
        ...
        I_Ca_junc = (Fjunc_CaL * ibarca_j * d_initvalu_4 * d_initvalu_5 *
                     (1 - d_initvalu_6) * dpct::pow(Q10CaL, Qpow)) *
                    0.45;
        I_Ca_sl = (Fsl_CaL * ibarca_sl * d_initvalu_4 * d_initvalu_5 *
                   (1 - d_initvalu_7) * dpct::pow(Q10CaL, Qpow)) *
                  0.45;
	...
	// I_ncx: Na/Ca Exchanger flux
        Ka_junc = 1 / (1 + dpct::pow((Kdact / d_initvalu_36), 3));
        Ka_sl = 1 / (1 + dpct::pow((Kdact / d_initvalu_37), 3));
        s1_junc = sycl::exp(nu * d_initvalu_39 * FoRT) *
                  dpct::pow(d_initvalu_32, 3) * Cao;
        s1_sl = sycl::exp(nu * d_initvalu_39 * FoRT) *
                dpct::pow(d_initvalu_33, 3) * Cao;
        s2_junc = sycl::exp((nu - 1) * d_initvalu_39 * FoRT) *
                  dpct::pow(Nao, 3) * d_initvalu_36;
        s3_junc = (KmCai * dpct::pow(Nao, 3) *
                       (1 + dpct::pow((d_initvalu_32 / KmNai), 3)) +
                   dpct::pow(KmNao, 3) * d_initvalu_36 +
                   dpct::pow(KmNai, 3) * Cao * (1 + d_initvalu_36 / KmCai) +
                   KmCao * dpct::pow(d_initvalu_32, 3) +
                   dpct::pow(d_initvalu_32, 3) * Cao +
                   dpct::pow(Nao, 3) * d_initvalu_36) *
                  (1 + ksat * sycl::exp((nu - 1) * d_initvalu_39 * FoRT));
        s2_sl = sycl::exp((nu - 1) * d_initvalu_39 * FoRT) * dpct::pow(Nao, 3) *
                d_initvalu_37;
        s3_sl = (KmCai * dpct::pow(Nao, 3) *
                     (1 + dpct::pow((d_initvalu_33 / KmNai), 3)) +
                 dpct::pow(KmNao, 3) * d_initvalu_37 +
                 dpct::pow(KmNai, 3) * Cao * (1 + d_initvalu_37 / KmCai) +
                 KmCao * dpct::pow(d_initvalu_33, 3) +
                 dpct::pow(d_initvalu_33, 3) * Cao +
                 dpct::pow(Nao, 3) * d_initvalu_37) *
                (1 + ksat * sycl::exp((nu - 1) * d_initvalu_39 * FoRT));
        I_ncx_junc = Fjunc * IbarNCX * dpct::pow(Q10NCX, Qpow) * Ka_junc *
                     (s1_junc - s2_junc) / s3_junc;
        I_ncx_sl = Fsl * IbarNCX * dpct::pow(Q10NCX, Qpow) * Ka_sl *
                   (s1_sl - s2_sl) / s3_sl;
	...
	// I_pca: Sarcolemmal Ca Pump Current
        I_pca_junc =
            Fjunc * dpct::pow(Q10SLCaP, Qpow) * IbarSLCaP *
            dpct::pow(d_initvalu_36, fp(1.6)) /
            (dpct::pow(KmPCa, fp(1.6)) + dpct::pow(d_initvalu_36, fp(1.6)));
        I_pca_sl =
            Fsl * dpct::pow(Q10SLCaP, Qpow) * IbarSLCaP *
            dpct::pow(d_initvalu_37, fp(1.6)) /
            (dpct::pow(KmPCa, fp(1.6)) + dpct::pow(d_initvalu_37, fp(1.6)));
	...
	MinSR = 1;
        kCaSR = MaxSR - (MaxSR - MinSR) /
                            (1 + dpct::pow(ec50SR / d_initvalu_31, fp(2.5)));
        koSRCa = koCa/kCaSR;
	kiSRCa = kiCa*kCaSR;
	RI = 1-d_initvalu_14-d_initvalu_15-d_initvalu_16;
        ...
        J_serca = dpct::pow(Q10SRCaP, Qpow) * Vmax_SRCaP *
                  (dpct::pow((d_initvalu_38 / Kmf), hillSRCaP) -
                   dpct::pow((d_initvalu_31 / Kmr), hillSRCaP)) /
                  (1 + dpct::pow((d_initvalu_38 / Kmf), hillSRCaP) +
                   dpct::pow((d_initvalu_31 / Kmr), hillSRCaP));
