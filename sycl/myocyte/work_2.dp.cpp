...
int work_2(int xmax, int workload) {
 ...
 sycl::queue &q_ct1 = dev_ct1.in_order_queue();
	...
	DPCT1049:6: The work-group size passed to the SYCL kernel may
         ...
         * needed.
	*/
        q_ct1.parallel_for(sycl::nd_range<3>(blocks * threads, threads),
                           [=](sycl::nd_item<3> item_ct1) {
                                   solver_2(workload, xmax, d_x, d_y, d_params,
                                            d_com, d_err, d_scale, d_yy,
                                            d_initvalu_temp, d_finavalu_temp,
                                            item_ct1);
                           });
	...
	free(y);
        dpct::dpct_free(d_y, q_ct1);

        free(x);
        dpct::dpct_free(d_x, q_ct1);

        free(params);
        dpct::dpct_free(d_params, q_ct1);

        //============================================================60
	//		TEMPORARY SOLVER VARIABLES
	//============================================================60

        dpct::dpct_free(d_com, q_ct1);

        dpct::dpct_free(d_err, q_ct1);
        dpct::dpct_free(d_scale, q_ct1);
        dpct::dpct_free(d_yy, q_ct1);

        dpct::dpct_free(d_initvalu_temp, q_ct1);
        dpct::dpct_free(d_finavalu_temp, q_ct1);
