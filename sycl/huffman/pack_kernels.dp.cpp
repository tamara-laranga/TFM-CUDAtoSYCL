...
static void pack2(unsigned int *srcData, unsigned int *cindex, unsigned int *cindex2, unsigned int *dstData, unsigned int original_num_block_elements,
        ...
        unsigned int tid = item_ct1.get_local_range(2) * item_ct1.get_group(2) +
	...
	tmp = dw >> bit;				// cut off those bits that do not fit into the initial location in destData[]
        dpct::atomic_fetch_or<sycl::access::address_space::generic_space>(
            &dstData[dword], tmp); // fill up this initial location
	...
	if (bit != 0 || bitsize % 32 != 0)
		...
		dw = srcData[offset+i];
                dpct::atomic_fetch_or<
                    sycl::access::address_space::generic_space>(
                    &dstData[dword + i], dw >> bit);
                dpct::atomic_fetch_or<
                    sycl::access::address_space::generic_space>(
                    &dstData[dword + i + 1], dw << 32 - bit);
        }
}
