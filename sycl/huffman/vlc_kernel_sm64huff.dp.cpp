...
static void vlc_encode_kernel_sm64huff(unsigned int* data,
        ...
        unsigned int kn = item_ct1.get_group(2) * item_ct1.get_local_range(2) +
    ...
    for (unsigned int d = (item_ct1.get_local_range(2)) >> 1; d > 0; d >>= 1) {
        /*
    ...
    if (k == 0) as[item_ct1.get_local_range(2) - 1] = 0;

    // traverse down the tree building the scan in place
    for (unsigned int d = 1; d < item_ct1.get_local_range(2); d *= 2) {
        offset >>= 1;
        /*
        DPCT1118:1: SYCL group functions and algorithms must be
         * encountered in converged control flow. You may need to adjust the
         * code.
        */
        item_ct1.barrier(sycl::access::fence_space::local_space);
        ...
        if (k == item_ct1.get_local_range(2) - 1) {
	...
	//else
        dpct::atomic_fetch_or<sycl::access::address_space::generic_space>(
            &as[kc + 2], tmpcw32 << (32 - codewordlen));
        }
