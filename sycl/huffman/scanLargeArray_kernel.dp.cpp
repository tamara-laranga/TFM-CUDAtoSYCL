                                       ...
                                       const sycl::nd_item<3> &item_ct1)
{
    ...
    mem_bi = mem_ai + item_ct1.get_local_range(2);

    ai = thid;
    bi = thid + item_ct1.get_local_range(2);
                                 ...
                                 const sycl::nd_item<3> &item_ct1)
{
    ...
    {
        int index = (item_ct1.get_local_range(2) << 1) - 1;
...
static unsigned int buildSum(unsigned int *s_data,
                             const sycl::nd_item<3> &item_ct1)
{
    ...
    for (int d = item_ct1.get_local_range(2); d > 0; d >>= 1)
    {
        /*
        DPCT1118:2: SYCL group functions and algorithms must be
                             ...
                             const sycl::nd_item<3> &item_ct1)
{
    ...
    for (int d = 1; d <= item_ct1.get_local_range(2); d *= 2)
    {
        stride >>= 1;
                           ...
                           unsigned int &uni)
{
    ...
    */
    ...
    g_data[address]              += uni;
    g_data[address + item_ct1.get_local_range(2)] +=
        (item_ct1.get_local_id(2) + item_ct1.get_local_range(2) < n) * uni;
}
