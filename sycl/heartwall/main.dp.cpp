...
static dpct::constant_memory<params_common_change, 0> d_common_change;

params_common common;
static dpct::constant_memory<params_common, 0> d_common;

params_unique unique[ALL_POINTS];								// cannot determine size dynamically so choose more than usually needed
static dpct::constant_memory<params_unique, 1> d_unique(ALL_POINTS);
 ...
 sycl::queue &q_ct1 = dev_ct1.in_order_queue();
  ...
  printf("WG size of kernel = %d \n", NUMBER_THREADS);
	...
	//====================================================================================================
        ...
        q_ct1.memcpy(d_common.get_ptr(), &common, sizeof(params_common));
        q_ct1
	...
	for(common_change.frame_no=0; common_change.frame_no<frames_processed; common_change.frame_no++){
		...
		// copy frame to GPU memory
                q_ct1.memcpy(common_change.d_frame, frame, common.frame_mem);
                q_ct1
                ...
                // launch GPU kernel
                /*
		DPCT1049:35: The work-group size passed to the
		...
		*/
                {
                        ...
                        q_ct1.submit([&](sycl::handler &cgh) {
                                ...
                                sycl::local_accessor<float, 1>
                                    in_partial_sum_acc_ct1(sycl::range<1>(51),
                                                           cgh);
                                sycl::local_accessor<float, 1>
                                    in_sqr_partial_sum_acc_ct1(
                                        sycl::range<1>(51), cgh);
                                sycl::local_accessor<float, 0>
                                    in_final_sum_acc_ct1(cgh);
                                sycl::local_accessor<float, 0>
                                    in_sqr_final_sum_acc_ct1(cgh);
                                sycl::local_accessor<float, 0> denomT_acc_ct1(
                                    cgh);
                                sycl::local_accessor<float, 1>
                                    par_max_val_acc_ct1(sycl::range<1>(131),
                                                        cgh);
                                sycl::local_accessor<int, 1>
                                    par_max_coo_acc_ct1(sycl::range<1>(131),
                                                        cgh);
                                sycl::local_accessor<float, 1>
                                    d_in_mod_temp_acc_ct1(sycl::range<1>(2601),
                                                          cgh);

                                cgh.parallel_for(
                                    sycl::nd_range<3>(blocks * threads,
                                                      threads),
                                    [=](sycl::nd_item<3> item_ct1) {
                                            kernel(item_ct1,
                                                   *d_common_change_ptr_ct1,
                                                   *d_common_ptr_ct1,
                                                   d_unique_ptr_ct1,
                                                   in_partial_sum_acc_ct1
                                                       .get_multi_ptr<
                                                           sycl::access::
                                                               decorated::no>()
                                                       .get(),
                                                   in_sqr_partial_sum_acc_ct1
                                                       .get_multi_ptr<
                                                           sycl::access::
                                                               decorated::no>()
                                                       .get(),
                                                   in_final_sum_acc_ct1,
                                                   in_sqr_final_sum_acc_ct1,
                                                   denomT_acc_ct1,
                                                   par_max_val_acc_ct1
                                                       .get_multi_ptr<
                                                           sycl::access::
	...
	// frame
        dpct::dpct_free(common_change.d_frame, q_ct1);
	...
	free(common.tEndoColLoc);
        ...
        dpct::dpct_free(common.d_endoRow, q_ct1);
        dpct::dpct_free(common.d_endoCol, q_ct1);
        dpct::dpct_free(common.d_tEndoRowLoc, q_ct1);
        dpct::dpct_free(common.d_tEndoColLoc, q_ct1);

        dpct::dpct_free(common.d_endoT, q_ct1);
	...
	free(common.tEpiColLoc);
        ...
        dpct::dpct_free(common.d_epiRow, q_ct1);
        dpct::dpct_free(common.d_epiCol, q_ct1);
        dpct::dpct_free(common.d_tEpiRowLoc, q_ct1);
        dpct::dpct_free(common.d_tEpiColLoc, q_ct1);

        dpct::dpct_free(common.d_epiT, q_ct1);
	...
	for(i=0; i<common.allPoints; i++){
                dpct::dpct_free(unique[i].d_in2, q_ct1);

                dpct::dpct_free(unique[i].d_conv, q_ct1);
                dpct::dpct_free(unique[i].d_in2_pad_cumv, q_ct1);
                dpct::dpct_free(unique[i].d_in2_pad_cumv_sel, q_ct1);
                dpct::dpct_free(unique[i].d_in2_sub_cumh, q_ct1);
                dpct::dpct_free(unique[i].d_in2_sub_cumh_sel, q_ct1);
                dpct::dpct_free(unique[i].d_in2_sub2, q_ct1);
                dpct::dpct_free(unique[i].d_in2_sqr, q_ct1);
                dpct::dpct_free(unique[i].d_in2_sqr_sub2, q_ct1);
                dpct::dpct_free(unique[i].d_in_sqr, q_ct1);

                dpct::dpct_free(unique[i].d_tMask, q_ct1);
                dpct::dpct_free(unique[i].d_mask_conv, q_ct1);
        }
