...
int main(int argc, char *argv []){
	...
	mem_size = sizeof(fp) * Ne;																		// get the size of float representation of input IMAGE
        d_I = (float *)sycl::malloc_device(mem_size, dpct::get_in_order_queue()); //

        // allocate memory for coordinates on DEVICE
        d_iN = (int *)sycl::malloc_device(mem_size_i, dpct::get_in_order_queue()); //
        dpct::get_in_order_queue().memcpy(d_iN, iN, mem_size_i).wait();            //
        d_iS = (int *)sycl::malloc_device(mem_size_i, dpct::get_in_order_queue()); //
        dpct::get_in_order_queue().memcpy(d_iS, iS, mem_size_i).wait();            //
        d_jE = (int *)sycl::malloc_device(mem_size_j, dpct::get_in_order_queue()); //
        dpct::get_in_order_queue().memcpy(d_jE, jE, mem_size_j).wait();            //
        d_jW = (int *)sycl::malloc_device(mem_size_j, dpct::get_in_order_queue()); //
        dpct::get_in_order_queue().memcpy(d_jW, jW, mem_size_j).wait();            //

        // allocate memory for partial sums on DEVICE
        d_sums = (float *)sycl::malloc_device(mem_size, dpct::get_in_order_queue()); //
        d_sums2 = (float *)sycl::malloc_device(mem_size,
                                               dpct::get_in_order_queue()); //

        // allocate memory for derivatives
        d_dN = (float *)sycl::malloc_device(mem_size, dpct::get_in_order_queue()); //
        d_dS = (float *)sycl::malloc_device(mem_size, dpct::get_in_order_queue()); //
        d_dW = (float *)sycl::malloc_device(mem_size, dpct::get_in_order_queue()); //
        d_dE = (float *)sycl::malloc_device(mem_size, dpct::get_in_order_queue()); //

        // allocate memory for coefficient on DEVICE
        d_c = (float *)sycl::malloc_device(mem_size, dpct::get_in_order_queue()); //
	...
	//================================================================================80
        ...
        dpct::get_in_order_queue().memcpy(d_I, image, mem_size).wait();
	...
	*/
	...
	// fflush(NULL);
		...
		// execute square kernel
                /*
		DPCT1049:5: The work-group size passed to the
		...
		*/
		...
		mul = 1;														// original multiplier
			...
			// run kernel
                        /*
			DPCT1049:8: The work-group size
			...
			*/
                        dpct::get_in_order_queue().submit(
                            [&](sycl::handler &cgh) {
                                    /*
			    DPCT1101:18:
                                     * 'NUMBER_THREADS' expression was replaced
                                     * with a value. Modify the code to use the
                                     * original expression, provided in
                                     * comments, if it is correct.

                                     */
                                    sycl::local_accessor<fp, 1> d_psum_acc_ct1(
			    ...
			    DPCT1101:19:
                                     ...
                                     */
                                    sycl::local_accessor<fp, 1> d_psum2_acc_ct1(
                                        sycl::range<1>(NUMBER_THREADS),
                                        cgh);
		...
		mem_size_single = sizeof(fp) * 1;
                dpct::get_in_order_queue().memcpy(&total, d_sums, mem_size_single);
                dpct::get_in_order_queue().memcpy(&total2, d_sums2, mem_size_single).wait();
		...
		// execute srad kernel
                /*
		DPCT1049:6: The work-group size passed to the
                 * SYCL kernel may exceed the limit. To get the device limit,
                 * query info::device::max_work_group_size. Adjust the
                 * work-group size if needed.
		*/
                dpct::get_in_order_queue().parallel_for(
                    sycl::nd_range<3>(blocks * threads, threads),
                    [=](sycl::nd_item<3> item_ct1) {
                            srad(lambda, Nr, Nc, Ne, d_iN, d_iS, d_jE, d_jW,
                                 d_dN, d_dS, d_dW, d_dE, q0sqr, d_c, d_I,
                                 item_ct1);
                    }); // output image
		...
		// execute srad2 kernel
                /*
		DPCT1049:7: The work-group size passed to the
                 * SYCL kernel may exceed the limit. To get the device limit,
                 * query info::device::max_work_group_size. Adjust the
                 * work-group size if needed.
		*/
                dpct::get_in_order_queue().parallel_for(
                    sycl::nd_range<3>(blocks * threads, threads),
                    [=](sycl::nd_item<3> item_ct1) {
                            srad2(lambda, Nr, Nc, Ne, d_iN, d_iS, d_jE, d_jW,
                                  d_dN, d_dS, d_dW, d_dE, d_c, d_I, item_ct1);
                    }); // output image
	...
	*/
	...
	//================================================================================80
        ...
        dpct::get_in_order_queue().memcpy(image, d_I, mem_size).wait();
	...
	free(jE);
        ...
        dpct::dpct_free(d_iN, dpct::get_in_order_queue());
        dpct::dpct_free(d_iS, dpct::get_in_order_queue());
        dpct::dpct_free(d_jE, dpct::get_in_order_queue());
        dpct::dpct_free(d_jW, dpct::get_in_order_queue());
        dpct::dpct_free(d_dN, dpct::get_in_order_queue());
        dpct::dpct_free(d_dS, dpct::get_in_order_queue());
        dpct::dpct_free(d_dE, dpct::get_in_order_queue());
        dpct::dpct_free(d_dW, dpct::get_in_order_queue());
        dpct::dpct_free(d_sums, dpct::get_in_order_queue());
        dpct::dpct_free(d_sums2, dpct::get_in_order_queue());
