...
void find_index_kernel(double * arrayX, double * arrayY, double * CDF, double * u, double * xj, double * yj, double * weights, int Nparticles,
     ...
     * sycl::nd_item::barrier() if function "find_index_kernel" is called in a
     * multidimensional kernel.
    */
    item_ct1.barrier(sycl::access::fence_space::local_space); // Usa sycl::nd_item::barrier() en lugar de sycl::nd_item::barrier(sycl::access::fence_space::local_space)
}
...
void particleFilter(unsigned char *I, int IszX, int IszY, int Nfr, int *seed,
    ...
    check_error(DPCT_CHECK_ERROR(
        arrayX_GPU = sycl::malloc_device<double>(Nparticles, q_ct1)));
    check_error(DPCT_CHECK_ERROR(
        arrayY_GPU = sycl::malloc_device<double>(Nparticles, q_ct1)));
    check_error(DPCT_CHECK_ERROR(
        xj_GPU = sycl::malloc_device<double>(Nparticles, q_ct1)));
    check_error(DPCT_CHECK_ERROR(
        yj_GPU = sycl::malloc_device<double>(Nparticles, q_ct1)));
    check_error(DPCT_CHECK_ERROR(
        CDF_GPU = sycl::malloc_device<double>(Nparticles, q_ct1)));
    check_error(DPCT_CHECK_ERROR(
        u_GPU = sycl::malloc_device<double>(Nparticles, q_ct1)));
    check_error(DPCT_CHECK_ERROR(
        likelihood_GPU = sycl::malloc_device<double>(Nparticles, q_ct1)));
    //set likelihood to zero
    check_error(DPCT_CHECK_ERROR(
        q_ct1.memset((void *)likelihood_GPU, 0, sizeof(double) * Nparticles)
            .wait()));
    check_error(DPCT_CHECK_ERROR(
        weights_GPU = sycl::malloc_device<double>(Nparticles, q_ct1)));
    check_error(DPCT_CHECK_ERROR(
        I_GPU = (unsigned char *)sycl::malloc_device(
            sizeof(unsigned char) * IszX * IszY * Nfr, q_ct1)));
    check_error(DPCT_CHECK_ERROR(objxy_GPU = (int *)sycl::malloc_device(
                                     sizeof(int) * 2 * countOnes, q_ct1)));
    check_error(
        DPCT_CHECK_ERROR(ind_GPU = (int *)sycl::malloc_device(
                             sizeof(int) * countOnes * Nparticles, q_ct1)));
    check_error(DPCT_CHECK_ERROR(
        seed_GPU = sycl::malloc_device<int>(Nparticles, q_ct1)));
    check_error(DPCT_CHECK_ERROR(
        partial_sums = sycl::malloc_device<double>(Nparticles, q_ct1)));
    ...
    check_error(DPCT_CHECK_ERROR(
        q_ct1.memcpy(I_GPU, I, sizeof(unsigned char) * IszX * IszY * Nfr)
            .wait()));
    check_error(DPCT_CHECK_ERROR(
        q_ct1.memcpy(objxy_GPU, objxy, sizeof(int) * 2 * countOnes).wait()));
    check_error(DPCT_CHECK_ERROR(
        q_ct1.memcpy(weights_GPU, weights, sizeof(double) * Nparticles)
            .wait()));
    check_error(DPCT_CHECK_ERROR(
    ...
    check_error(DPCT_CHECK_ERROR(
        q_ct1.memcpy(seed_GPU, seed, sizeof(int) * Nparticles).wait()));
            ...
            q_ct1.submit([&](sycl::handler &cgh) {
                ...
                cgh.parallel_for(
                    ...
                    [=](sycl::nd_item<3> item_ct1) {
                        likelihood_kernel(
                            arrayX_GPU, arrayY_GPU, xj_GPU, yj_GPU, CDF_GPU,
                            ind_GPU, objxy_GPU, likelihood_GPU, I_GPU, u_GPU,
                            weights_GPU, Nparticles, countOnes, max_size, k,
                            IszY, Nfr, seed_GPU, partial_sums, item_ct1,
                            buffer_acc_ct1
            ...
            q_ct1.submit([&](sycl::handler &cgh) {
                ...
                cgh.parallel_for(
                    sycl::nd_range<3>(
                        ...
                        sycl::range<3>(1, 1, threads_per_block)),
                    [=](sycl::nd_item<3> item_ct1) {
                        normalize_weights_kernel(weights_GPU, Nparticles,
                                                 partial_sums, CDF_GPU, u_GPU,
                                                 seed_GPU, item_ct1, u1_acc_ct1,
                                                 sumWeights_acc_ct1);
                    });
            });
        }
    ...
    dpct::dpct_free(xj_GPU, q_ct1);
    dpct::dpct_free(yj_GPU, q_ct1);
    dpct::dpct_free(CDF_GPU, q_ct1);
    dpct::dpct_free(u_GPU, q_ct1);
    dpct::dpct_free(likelihood_GPU, q_ct1);
    dpct::dpct_free(I_GPU, q_ct1);
    dpct::dpct_free(objxy_GPU, q_ct1);
    dpct::dpct_free(ind_GPU, q_ct1);
    dpct::dpct_free(seed_GPU, q_ct1);
    dpct::dpct_free(partial_sums, q_ct1);
    ...
    check_error(DPCT_CHECK_ERROR(
        q_ct1.memcpy(weights, weights_GPU, sizeof(double) * Nparticles)
            .wait()));
    ...
    printf("YE: %lf\n", ye);
    double distance =
        sqrt(dpct::pow((double)(xe - (int)roundDouble(IszY / 2.0)), 2) +
             dpct::pow((double)(ye - (int)roundDouble(IszX / 2.0)), 2));
    printf("%lf\n", distance);

    //CUDA freeing of memory
    dpct::dpct_free(weights_GPU, q_ct1);
    dpct::dpct_free(arrayY_GPU, q_ct1);
    dpct::dpct_free(arrayX_GPU, q_ct1);
