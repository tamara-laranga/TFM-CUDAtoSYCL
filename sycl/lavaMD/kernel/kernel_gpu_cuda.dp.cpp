 ...
 * kernel_gpu_cuda exceeds 128 bytes and may cause high register pressure.
		...
		*/
		...
		for (k=0; k<(1+d_box_gpu[bx].nn); k++){
			...
			// synchronize threads because in next section each thread accesses data brought in by different threads here
                        /*
			DPCT1118:2: SYCL group functions
			...
			*/
			...
			// synchronize after finishing force contributions from current nei box not to cause conflicts when starting next box
                        /*
			DPCT1118:3: SYCL group functions
                         * and algorithms must be encountered in converged
                         * control flow. You may need to adjust the code.

                         */
                        /*
			DPCT1065:12: Consider replacing
                         * sycl::nd_item::barrier() with
                         * sycl::nd_item::barrier(sycl::access::fence_space::local_space)
                         * for better performance if there is no access to
                         * global memory.
